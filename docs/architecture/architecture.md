...
### Game Server - концентрация нагрузки реального времени

**Проблема.** Каждый бой требует постоянного WebSocket-соединения с двумя Агентами и вычислительных ресурсов для обработки команд. При росте числа одновременных боев один экземпляр не справится.

**Решение.** Горизонтальное масштабирование. Бои изолированы друг от друга, состояние одного боя не требуется другому. При старте боя Tournament Service выбирает наименее загруженный экземпляр Game Server и сообщает Агентам его адрес. Агенты подключаются напрямую к этому экземпляру.

### BattleHistory Service - высокая нагрузка на чтение

**Проблема.** Записи боев пишутся редко (бой завершен - одна запись), а читаются часто (игроки смотрят прошедшие бои). Объем данных со временем растет.

**Решение.** Разделение на чтение и запись (CQRS). Данные боя сохраняются в основное хранилище, оптимизированное под запись, затем асинхронно проецируются в хранилище для чтения. Для горячих данных (недавние бои) используется Redis, для холодных (старые бои) - объектное хранилище (S3) с отложенной загрузкой.

### Message Broker - единая точка отказа

**Проблема.** Все асинхронные события проходят через брокер. Если он недоступен, уведомления не доставляются, рейтинги не обновляются, история боев не сохраняется. Бои продолжаются, но данные теряются или задерживаются.

**Решение.** Кластеризация брокера (репликация). Сервисы-отправители сохраняют события локально (outbox pattern) и публикуют их при восстановлении брокера. Сервисы-получатели идемпотентны: повторная обработка того же события не приводит к дубликатам.
